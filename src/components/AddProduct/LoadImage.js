...
export const LoadImage = ({setFile}) => {
    ...
    const readFile = async (file) => {
        ...
        const promise = new Promise ( async (resolve, reject) => {
            ...
            console.log(status)
            if(status === 204) { resolve({link: `https://d1d5i0xjsb5dtw.cloudfront.net/${file.name}`, format: file.type}) } else {reject("error loading to S3")} 
        });
